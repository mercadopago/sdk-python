...
class TestMerchantOrder(unittest.TestCase):
    ...
    def test_all(self):
        ...
        merchant_order_updated = self.sdk.merchant_order().update(merchant_order_created["response"]["id"], {"additional_info": "Info 2"})
        self.assertEqual(merchant_order_updated["status"], 200)

        merchant_order_finded = self.sdk.merchant_order().get(merchant_order_created["response"]["id"])
